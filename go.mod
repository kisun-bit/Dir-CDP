module jingrongshuan/rongan-fnotify

go 1.15

require (
	github.com/astaxie/beego v1.12.3
	github.com/boltdb/bolt v1.3.1
	github.com/cpuguy83/go-md2man/v2 v2.0.2 // indirect
	github.com/gin-gonic/gin v1.7.7
	github.com/go-openapi/spec v0.20.6 // indirect
	github.com/go-openapi/swag v0.21.1 // indirect
	github.com/go-ping/ping v1.0.0 // indirect
	github.com/go-playground/validator/v10 v10.11.0 // indirect
	github.com/golang/protobuf v1.5.2 // indirect
	github.com/journeymidnight/aws-sdk-go v1.18.2
	github.com/json-iterator/go v1.1.12 // indirect
	github.com/kisun-bit/go-dr v0.1.13-0.20220321082859-ef0bbe139746
	github.com/kisun-bit/go-s3 v0.0.0-20220321053947-bbf0cb8edb73
	github.com/kr/pretty v0.3.0
	github.com/mailru/easyjson v0.7.7 // indirect
	github.com/mattn/go-isatty v0.0.14 // indirect
	github.com/panjf2000/ants/v2 v2.5.0
	github.com/ryanuber/go-glob v1.0.0
	github.com/swaggo/files v0.0.0-20210815190702-a29dd2bc99b2 // indirect
	github.com/swaggo/gin-swagger v1.4.3 // indirect
	github.com/ugorji/go v1.2.7 // indirect
	github.com/unknwon/com v1.0.1
	github.com/urfave/cli/v2 v2.6.0 // indirect
	golang.org/x/crypto v0.0.0-20220511200225-c6db032c6c88 // indirect
	golang.org/x/sys v0.0.0-20220503163025-988cb79eb6c6 // indirect
	golang.org/x/tools v0.1.10 // indirect
	google.golang.org/protobuf v1.28.0 // indirect
	gorm.io/driver/postgres v1.3.5
	gorm.io/gorm v1.23.5
)
